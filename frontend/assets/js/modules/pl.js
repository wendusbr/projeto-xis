...
function simplexMax(entrada){
    ...
    while(1){
        ...
        for(let i=1; i<=tabelaSimplex.length-1; i++){
        ...
        writeMaxTableFront(it, tabelaSimplex, linhaPivotal, colunaPivotal);
...
function simplexMin(entrada){
    ...
    while(1){        
        ...
        writeMinTableFront(it, z, c, tabelaSimplex, zj, pesoZj, linhaPivotal, colunaPivotal);
...
function writeHeaderMaxTableFront(tabelaSimplex){
    ...
    tableHeader += '<tr class="fw-bold"><td>Z</td>';

    for(let i=1; i<tabelaSimplex[0].length-1; i++){
        tableHeader += '<td>';
        switch(tabelaSimplex[0][i]){
            case 0:
                tableHeader += 'SorA' ;
                break;
        ...
        tableHeader += `<span class="subindice">${i}</span></td>`;
    }

    tableHeader += '<td>RHS</td></tr>';
}

function writeMaxTableFront(it, tabelaSimplex, linhaPivotal, colunaPivotal){
    ...
    if(it == 0)
        writeHeaderMaxTableFront(tabelaSimplex);
...
function writeMinTableFront(it, z, c, tabelaSimplex, zj, pesoZj, linhaPivotal, colunaPivotal){
    // Título e inicialização da tabela
    let table = `
    ...
    table += '<td style="font-size: 12px;">X: variável problema<br>S: variável auxiliar<br>A: variável artificial</td></tr>';
